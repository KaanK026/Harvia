...
async def chat_stream(chat_chain, question: str, session_id: str):
    """
    Stream a response from the memory-aware chat chain.
    Uses astream_events to capture token-level streaming from the LLM.

    Args:
        chat_chain: The initialized QA chain with streaming enabled
        question: User's question
        session_id: REQUIRED unique identifier for the user/session

    Yields:
        dict: Streaming chunks with format:
            - {"type": "token", "content": str} for text chunks
            - {"type": "sources", "sources": List[str]} when sources are available
            - {"type": "done", "session_id": str, "chat_history": List[dict]} when complete
    """
    if chat_chain is None:
        logger.error("Chat chain is None. Cannot process the question.")
        yield {"type": "error", "content": "Chat chain is not initialized"}
        return

    try:
        logger.info(f"Streaming question: {question} for session: {session_id}")
        ...
        # Use astream_events to capture token-level streaming from the LLM
        # This gives us access to individual token events even through the chain
        try:
            # Try with v2 first, fallback to v1 if not supported
            try:
                events = chat_chain.astream_events(
                    {"input": question},
                    config={"configurable": {"session_id": session_id}},
                    version="v2"
                )
            except (TypeError, ValueError):
                # Fallback to v1 if v2 not supported
                events = chat_chain.astream_events(
                    {"input": question},
                    config={"configurable": {"session_id": session_id}}
                )
            ...
            async for event in events:
                ...
                # Capture individual tokens from LLM streaming
                ...
                # Capture sources from retrieval chain
                elif event_type == "on_chain_end":
                    data = event.get("data", {})
                    output = data.get("output", {})

                    # Check if this is the retrieval chain output with context
                    if isinstance(output, dict) and "context" in output:
                        context = output.get("context", [])
                        if context and not seen_sources:
                            for doc in context:
                                metadata = getattr(doc, "metadata", {})
                                source_path = metadata.get("source") or metadata.get("file_name") or "unknown"
                                sources_info.add(os.path.basename(source_path))
            ...
            # Fallback: if astream_events fails (e.g., version not supported), use astream
            logger.warning(f"astream_events not available, using astream fallback: {e}")
            last_answer = ""
            async for chunk in chat_chain.astream(
                ...
                current_answer = chunk.get("answer") or chunk.get("output") or ""

                # Extract sources from context
                sources = chunk.get("context", [])
                if sources and not seen_sources:
                    for doc in sources:
                        metadata = getattr(doc, "metadata", {})
                        source_path = metadata.get("source") or metadata.get("file_name") or "unknown"
                        sources_info.add(os.path.basename(source_path))
                ...
                # Only yield new content (incremental difference)
                if current_answer and current_answer != last_answer:
                    new_content = current_answer[len(last_answer):]
                    if new_content:
        ...
        if sources_info:
            yield {"type": "sources", "sources": list(sources_info)}
        ...
        yield {"type": "error", "content": f"Error processing question: {str(e)}"}
